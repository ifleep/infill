...
// src/lib/db.ts. This config file is CLI-only (migrate/introspect), which
// connects with a plain connection string via `datasource.url` instead.
//
...
// unset, so `generate` still succeeds; `migrate`, which does need a real
// connection, fails with Prisma's own clear
